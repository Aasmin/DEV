...
async function handleSinglePage(tab, browser) {
    ...
    let nxtBtn = allLis[allLis.length - 2]; //second last bnda mil jaega
    let isDisabled = await tab.evaluate(function (elem) {
        return elem.getAttribute("class");
    }, nxtBtn);
    if (isDisabled == "disabled") { //agar class lagi hoi disabled name di on the elem
        return;
    } else {
    ...
    return new Promise(function (resolve, reject) {
        newTab.goto(cLink, { waitUntil: "networkidle0" }).
            ...
            .then(function () {
                ...
                let keyWillEnteredP = newTab.type("#moderator", "aasmin1kainth");
